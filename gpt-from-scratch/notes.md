...
what max tokens ( or chars if the model si char based ) are on the input side

in char model, if context size is 8 => upto 8 chars can be used to predict the 9th
...
How many bit do u need to encode a dice throw, obviously log_2(2) = 1
...
note that thus log_2( counts ) is just the general case? what if probs are
unequal?
H(p) = entropy ( shannon entropy ) = - sum p(x) log_2 ( p(x) )
...
this is 3 times - 1/3 log2( 1/3 ) => - log2( 3^-1 ) => log2( 3 )
...
you get to H(p,q) = sum p(x) . I_q( x )
...
KL divergence => KL(p||q) is simply the extra cost from the wrong belief

H(p,q) - H(p) => + sum p(x) log( p(x) / q(x) )
...
now it's not that same logic func but ir just used for an unbounded range
that WILL later on be convered to probabs using softmax etc
...
Assume p(y|x) => char y after x is same for all => 1/65
we would reach to logits as ln(number of choices) = ln(65) ~= 4.17

this 4.17 is H(p,q) -> H(p)

Gibb's inequality H(p,q) >= H(p,p)
...
autograd graph
also `model.eval()` which changes behav of some layers like dropout but we
...
p(i) = exp( v_i ) / sum over exp( v_j )

_ multinomial selection _
weight by preds and select one sample

_ cat _
append that to context at 1 so the context varkeeps on growing but ofc
...
these d_q/k/v are the dimensions the query, keys and values live in
this is def meant to be <= C else you are "creating" more information
from less which is just wrong

Say we have three tokens in input -> x0, x1, and x2
to use those we use their embeddings emb(x0), emb(x1), emb(x2)

q, k, v operate in a smaller dimension usually so we a projection,
...
magic for math reasons -> S(i,j) /= root d_k
...
A - these preds we got post softmaxcool did we push it?
= attention matrix

note that this A is a lower left triangle, upper half is masked

now out_i => for each query token weight by probabds the values ( we USE
values )

out_i = sum over j   A(i,j) <- match score probab for jth key * jth value
...
"non-linearity" to it, even though it goes throw a whole W_q projection

_ why higher dims almost orthogonal _

dot product = sum over ui vi
ui and vi are independent, their products then are too, idea being you start at
0 and accumulate deltas = ui vi which is similar to a random walk of len d
so as there are more terms it's closer to 0
=> num grows as root d ( random walk grows as root d ) and when you norm
that vec the scalar becomes 1 / root d

so in input we get
x = token_emb + ( not concat ) pos_emb

_ broadcasting rules _

same in pytorch as they are in numpy
*todo*

_ to_logits ( lm head as it's called in bert and incorrectly names as such in
some impls ) _

this is used a projection to "vocab_size" post attention stuff
...
_ the three attributes for nn.Module _

the __setattr__ is overriden so an assignment if it's a
nn.Parameter or another module it gets paramters + auto move to devices +
state dict etc
...
a dot is idiomatically a @ b.transpose(dim a indx, dim b indx) where the
...
swaps out these two dimensions, since swap is symmetric transpose(a,b) =
transpose(b,a)

we earlier did tril == 0 for the mask step making it triu(1) 1 meaning
diagonals are 0ed too
...
note that A is the weight assigned to each position, not actualy probabs
of the next token
...
logits = W_lm . [ softmax( (W_q x)(W_k x)T / √d + mask ) . (W_v x) ]
|       |          |---------------- A(x), (T,T) --------|   |
|       readout                 routing weights              values

x = E_tokens + P_positions_       (B, T, C)
...
logits = W_lm . W_2 . ReLU( W_1 . [ A(x) . W_v x ] + b_1 ) + b_2
| | | | | | | | | | |--------- the only bend ---------|

A learns who is relevant to what
W_v learns it's contribution | given relevance
...
dims od the middle layer = counts of relu = count of non-linearities in there ( hinges )
ideally we want more of such bends but can't just increase C ( n_embd ) so emprically increase
where cheap, 4 is what the og paper used

**splitting attention**

a single attention head learns W_q,k,v off of random init, based on where it started of randomly
...
there os one more step done here in practice => another linear projection Wo
...
x1 = mha(x0)
x2 = ffn(x1)
logits = W_(lm_head I think I renamed it to something but yeah we want logic dim to be vocab size, this'll do a proj) . x2
...
I beleive it's just more params, something about signal being to "mixed" up when going deeper to predict anything
reasonable, or maybe the magnitudes diminish? ( todo, again )

depth without residuals does nto help and instead makes it worse

**residuals**

> offtopic: layernorm is not regularisation, I somehow got this idea that it is, batch norm is, somewhat atleast
...
also attention as convex combination ( sum p_i, pi >= 0, so just like probabs ) so it does a "weighted" smoothing across the
...
currently the last linear on mha and the first on on ffn are collapsed, idea was we add residuals here
...
CC just says -> "giving direct line to stream" and seem sall transformers do it this way
...
this here is 2 x blocks as each block has two, mha and ffn
...
===> prereq => change in mag and dire of sum of high dimensional vectors

x_L = sum over x_i
where each x_i is in R_n

if any two vecs are ALMOST orthogonal
dot product is a sum of random walks, so u . v -> 0
simply E[u.v] -> 0 and std dev[u.v] = 1 / root C ( todo: derive )
...
grows in root k => k is the number of times we add
